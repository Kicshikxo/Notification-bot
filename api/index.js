...
app.post('/dispatch', async (req, res) => {
	...
	if (!message.trim()) {
		return res.json({ success: false, error: new Error('MESSAGE_MUST_BE_NOT_EMPTY').toString() })
	} else if (!discord && !telegram) {
		return res.json({ success: false, error: new Error('NO_NOTIFICATION_TARGETS_SPECIFIED').toString() })
	}
